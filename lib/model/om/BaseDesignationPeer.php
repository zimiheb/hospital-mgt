...
abstract class BaseDesignationPeer {
	...
	public static function doCountJoinDepartment(Criteria $criteria, $distinct = false, PropelPDO $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		$criteria->addJoin(array(DesignationPeer::DEPARTMENT_ID,), array(DepartmentPeer::ID,), $join_behavior);
	...
	/**
	 * Selects a collection of Designation objects pre-filled with their Department objects.
	 * @param      Criteria  $c
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	 * @return     array Array of Designation objects.
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 */
    ...
    foreach (sfMixer::getCallables('BaseDesignationPeer:doSelectJoin:doSelectJoin') as $callable)
    {
      call_user_func($callable, 'BaseDesignationPeer', $c, $con);
    }
		...
		$startcol = (DesignationPeer::NUM_COLUMNS - DesignationPeer::NUM_LAZY_LOAD_COLUMNS);
		DepartmentPeer::addSelectColumns($c);

		$c->addJoin(array(DesignationPeer::DEPARTMENT_ID,), array(DepartmentPeer::ID,), $join_behavior);
		$stmt = BasePeer::doSelect($c, $con);
		$results = array();

		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$key1 = DesignationPeer::getPrimaryKeyHashFromRow($row, 0);
			if (null !== ($obj1 = DesignationPeer::getInstanceFromPool($key1))) {
				// We no longer rehydrate the object, since this can cause data loss.
				// See http://propel.phpdb.org/trac/ticket/509
				...
				$obj1 = new $cls();
				$obj1->hydrate($row);
				DesignationPeer::addInstanceToPool($obj1, $key1);
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (Designation) to $obj2 (Department)
				$obj2->addDesignation($obj1);
	...
	/**
	 * Selects a collection of Designation objects pre-filled with all related objects.
	 *
	 * @param      Criteria  $c
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
    ...
    foreach (sfMixer::getCallables('BaseDesignationPeer:doSelectJoinAll:doSelectJoinAll') as $callable)
    {
      call_user_func($callable, 'BaseDesignationPeer', $c, $con);
    }
		...
		DesignationPeer::addSelectColumns($c);
		$startcol2 = (DesignationPeer::NUM_COLUMNS - DesignationPeer::NUM_LAZY_LOAD_COLUMNS);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (Designation) to the collection in $obj2 (Department)
				$obj2->addDesignation($obj1);
  ...
  static public function getUniqueColumnNames()
  {
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += DesignationPeer::doOnDeleteCascade(new Criteria(DesignationPeer::DATABASE_NAME), $con);
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		try {
			...
			// for more than one table or we could emulating ON DELETE CASCADE, etc.
			$con->beginTransaction();
			$affectedRows += DesignationPeer::doOnDeleteCascade($criteria, $con);
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		$objects = DesignationPeer::doSelect($criteria, $con);
		foreach ($objects as $obj) {
			...
			$c->add(EmployeePeer::DESIGNATION_ID, $obj->getId());
