...
abstract class BaseVisit extends BaseObject  implements Persistent {
  ...
  const PEER = 'VisitPeer';
	...
	/**
	 * The value for the admit_date field.
	 * @var        string
	 */
	protected $admit_date;
	...
	/**
	 * @var        array VisitMedicine[] Collection to store aggregation of VisitMedicine objects.
	 */
	protected $collVisitMedicines;

	/**
	 * @var        Criteria The criteria used to select the current contents of collVisitMedicines.
	 */
	private $lastVisitMedicineCriteria = null;
	...
	/**
	 * Get the [optionally formatted] temporal [admit_date] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getAdmitDate($format = 'Y-m-d')
	{
		if ($this->admit_date === null) {
			return null;
		}


		if ($this->admit_date === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->admit_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->admit_date, true), $x);
			}
		}
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getDischargeDate($format = 'Y-m-d')
	{
		...
		if ($this->discharge_date === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->discharge_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->discharge_date, true), $x);
			}
		}
	...
	/**
	 * Sets the value of [admit_date] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Visit The current object (for fluent API support)
	 */
	public function setAdmitDate($v)
	{
		// we treat '' as NULL for temporal objects because DateTime('') == DateTime('now')
		// -- which is unexpected, to say the least.
		if ($v === null || $v === '') {
		...
		if ( $this->admit_date !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->admit_date !== null && $tmpDt = new DateTime($this->admit_date)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->admit_date = ($dt ? $dt->format('Y-m-d') : null);
				$this->modifiedColumns[] = VisitPeer::ADMIT_DATE;
			}
		} // if either are not null
	...
	public function setDischargeDate($v)
	{
		...
		if ( $this->discharge_date !== null || $dt !== null ) {
			...
			$currNorm = ($this->discharge_date !== null && $tmpDt = new DateTime($this->discharge_date)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->discharge_date = ($dt ? $dt->format('Y-m-d') : null);
				$this->modifiedColumns[] = VisitPeer::DISCHARGE_DATE;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->bp = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->temp = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->pulse = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->diet = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->description = ($row[$startcol + 10] !== null) ? (string) $row[$startcol + 10] : null;
			$this->time = ($row[$startcol + 11] !== null) ? (string) $row[$startcol + 11] : null;
			$this->visit_date = ($row[$startcol + 12] !== null) ? (string) $row[$startcol + 12] : null;
			$this->admit_date = ($row[$startcol + 13] !== null) ? (string) $row[$startcol + 13] : null;
			$this->discharge_date = ($row[$startcol + 14] !== null) ? (string) $row[$startcol + 14] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collVisitMedicines = null;
			$this->lastVisitMedicineCriteria = null;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[6] => $this->getBp(),
			$keys[7] => $this->getTemp(),
			$keys[8] => $this->getPulse(),
			$keys[9] => $this->getDiet(),
			$keys[10] => $this->getDescription(),
			$keys[11] => $this->getTime(),
			$keys[12] => $this->getVisitDate(),
			$keys[13] => $this->getAdmitDate(),
			$keys[14] => $this->getDischargeDate(),
			$keys[15] => $this->getFee(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[6], $arr)) $this->setBp($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setTemp($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setPulse($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setDiet($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setDescription($arr[$keys[10]]);
		if (array_key_exists($keys[11], $arr)) $this->setTime($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setVisitDate($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setAdmitDate($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setDischargeDate($arr[$keys[14]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(VisitPeer::ADMIT_DATE)) $criteria->add(VisitPeer::ADMIT_DATE, $this->admit_date);
		if ($this->isColumnModified(VisitPeer::DISCHARGE_DATE)) $criteria->add(VisitPeer::DISCHARGE_DATE, $this->discharge_date);
	...
	/**
	 * Clears out the collVisitMedicines collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addVisitMedicines()
	 */
	public function clearVisitMedicines()
	{
		$this->collVisitMedicines = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collVisitMedicines collection (array).
	 *
	 * By default this just sets the collVisitMedicines collection to an empty array (like clearcollVisitMedicines());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initVisitMedicines()
	{
		$this->collVisitMedicines = array();
	}

	/**
	 * Gets an array of VisitMedicine objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Visit has previously been saved, it will retrieve
	 * related VisitMedicines from storage. If this Visit is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array VisitMedicine[]
	 * @throws     PropelException
	 */
	public function getVisitMedicines($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(VisitPeer::DATABASE_NAME);
		}
		...
		if ($this->collVisitMedicines === null) {
			if ($this->isNew()) {
			   $this->collVisitMedicines = array();
				...
				$criteria->add(VisitMedicinePeer::VISIT_ID, $this->id);

				VisitMedicinePeer::addSelectColumns($criteria);
				$this->collVisitMedicines = VisitMedicinePeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				VisitMedicinePeer::addSelectColumns($criteria);
				if (!isset($this->lastVisitMedicineCriteria) || !$this->lastVisitMedicineCriteria->equals($criteria)) {
					$this->collVisitMedicines = VisitMedicinePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastVisitMedicineCriteria = $criteria;
		return $this->collVisitMedicines;
	}

	/**
	 * Returns the number of related VisitMedicine objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related VisitMedicine objects.
	 * @throws     PropelException
	 */
	public function countVisitMedicines(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(VisitPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collVisitMedicines === null) {
			if ($this->isNew()) {
				...
				$count = VisitMedicinePeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastVisitMedicineCriteria) || !$this->lastVisitMedicineCriteria->equals($criteria)) {
					...
					$count = count($this->collVisitMedicines);
				}
			} else {
				$count = count($this->collVisitMedicines);
			}
	...
	/**
	 * Method called to associate a VisitMedicine object to this object
	 * through the VisitMedicine foreign key attribute.
	 *
	 * @param      VisitMedicine $l VisitMedicine
	 * @return     void
	 * @throws     PropelException
	 */
	public function addVisitMedicine(VisitMedicine $l)
	{
		if ($this->collVisitMedicines === null) {
			$this->initVisitMedicines();
		}
		if (!in_array($l, $this->collVisitMedicines, true)) { // only add it if the **same** object is not already associated
			array_push($this->collVisitMedicines, $l);
			$l->setVisit($this);
		}
	}
	 ...
	 * Otherwise if this Visit is new, it will return
	 * an empty collection; or if this Visit has previously
	 * been saved, it will retrieve related VisitMedicines from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Visit.
	 */
	public function getVisitMedicinesJoinPatient($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(VisitPeer::DATABASE_NAME);
		}
		...
		if ($this->collVisitMedicines === null) {
			if ($this->isNew()) {
				...
				$this->collVisitMedicines = VisitMedicinePeer::doSelectJoinPatient($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * actually need in Visit.
	 */
	public function getVisitMedicinesJoinPharma($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(VisitPeer::DATABASE_NAME);
		}
		...
		if ($this->collVisitMedicines === null) {
			if ($this->isNew()) {
				...
				$this->collVisitMedicines = VisitMedicinePeer::doSelectJoinPharma($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * actually need in Visit.
	 */
	public function getVisitMedicinesJoinDosage($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(VisitPeer::DATABASE_NAME);
		}
		...
		if ($this->collVisitMedicines === null) {
			if ($this->isNew()) {
				...
				$this->collVisitMedicines = VisitMedicinePeer::doSelectJoinDosage($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collVisitMedicines) {
				foreach ((array) $this->collVisitMedicines as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
