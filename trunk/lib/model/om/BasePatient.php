...
abstract class BasePatient extends BaseObject  implements Persistent {
  ...
  const PEER = 'PatientPeer';
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Patient has previously been saved, it will retrieve
	 * related VisitMedicines from storage. If this Patient is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collVisitMedicines === null) {
			if ($this->isNew()) {
			   $this->collVisitMedicines = array();
				...
				$criteria->add(VisitMedicinePeer::PATIENT_ID, $this->id);
	 ...
	 * actually need in Patient.
	 */
	public function getVisitMedicinesJoinVisit($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PatientPeer::DATABASE_NAME);
		}
		...
		if ($this->collVisitMedicines === null) {
			if ($this->isNew()) {
				...
				$this->collVisitMedicines = VisitMedicinePeer::doSelectJoinVisit($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
