...
/**
 * Base static class for performing query and update operations on the 'room' table.
 *
 * 
 *
...
abstract class BaseRoomPeer {
	...
	/**
	 * An identiy map to hold any loaded instances of Room objects.
	 ...
	 * @param      string $column The column name for current table. (i.e. RoomPeer::COLUMN_NAME).
	 * @return     string
	 */
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(RoomPeer::TITLE);

		$criteria->addSelectColumn(RoomPeer::DESCRIPTION);

		$criteria->addSelectColumn(RoomPeer::PRICE);
    ...
    foreach (sfMixer::getCallables('BaseRoomPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseRoomPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseRoomPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseRoomPeer', $criteria, $con);
    }
	 ...
	 * @return     Room Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
  ...
  static public function getUniqueColumnNames()
  {
	...
	/**
	 * Method perform an INSERT on the database, given a Room or Criteria object.
	 *
	 * @param      mixed $values Criteria or Room object containing data that is used to create the INSERT statement.
    ...
    foreach (sfMixer::getCallables('BaseRoomPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseRoomPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
		...
		if ($values instanceof Criteria) {
			...
			$criteria = $values->buildCriteria(); // build Criteria from Room object
		}
    ...
    foreach (sfMixer::getCallables('BaseRoomPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseRoomPeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a Room or Criteria object.
	 *
	 * @param      mixed $values Criteria or Room object containing data that is used to create the UPDATE statement.
    ...
    foreach (sfMixer::getCallables('BaseRoomPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseRoomPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseRoomPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseRoomPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the room table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a Room or Criteria object OR a primary key value.
	 *
	...
	/**
	 * Validates all modified columns of given Room object.
	 * If parameter $columns is either a single column name or an array of column names
	 * than only those columns are validated.
	 *
	 * NOTICE: This does not apply to primary or foreign keys for now.
	 *
	 * @param      Room $obj The object to validate.
