...
abstract class BaseRole extends BaseObject  implements Persistent {
  ...
  const PEER = 'RolePeer';
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Role has previously been saved, it will retrieve
	 * related Users from storage. If this Role is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collUsers === null) {
			if ($this->isNew()) {
			   $this->collUsers = array();
				...
				$criteria->add(UserPeer::ROLE_ID, $this->id);
	 ...
	 * actually need in Role.
	 */
	public function getUsersJoinEmployee($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(RolePeer::DATABASE_NAME);
		}
		...
		if ($this->collUsers === null) {
			if ($this->isNew()) {
				...
				$this->collUsers = UserPeer::doSelectJoinEmployee($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
